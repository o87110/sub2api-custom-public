...
assert_line deploy/release/Dockerfile 'COPY --chown=sub2api:sub2api backend/resources /app/resources'
assert_line deploy/release/build-release-payload.sh 'cp -R "$repo_root/backend/resources" "$output/image-context/backend/resources"'
