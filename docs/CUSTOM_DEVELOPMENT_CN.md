...
| 官方基线 | `vendor-0.1.173^{commit}` |
| 基线提交 | `29009f0b2ea14edf3b11ae2564fb617ff91a03b4` |
...
精确审核，新增调用面按“路径 + 所属函数或模板事件位置 + 执行目标 + 次数”审核。
函数改名、普通条件分支、无控制流的多步骤辅助函数，以及在 Vendor 已有函数中追加的
直接、括号、可选或计算属性调用和模板事件处理器均不能绕过。
