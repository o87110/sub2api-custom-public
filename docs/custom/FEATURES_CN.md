...
本文档描述相对官方基线 `vendor-0.1.170` 的有效二改需求。文件级实现范围以
...
`NULL`，不会因升级改变销售行为；管理员填写时只能使用正整数，`0` 只由系统扣减
产生。

- 创建待支付订阅订单时在同一数据库事务内预占一份库存；条件更新保证并发请求不能
  把数量扣成负数，最后一份被预占后套餐自动下架；
...
- 订单创建时为不限量的套餐记录为 `untracked`，之后开启限量不追溯旧订单；全额和
  部分退款均不归还已消费数量；
- 管理更新数量采用三态语义：省略表示不修改，`null` 改为不限量，正整数设置当前
  剩余数量。自动下架套餐补货或改为不限量后自动恢复，手动下架套餐不自动恢复；
- 管理表格显示“无限制”、正整数或“0 / 已售罄”，并标识系统自动下架；售罄套餐
  必须先补货或改为不限量，不能直接手动上架；公开套餐列表同时过滤下架和售罄数据；
- 本功能新增 Migration `194_subscription_plan_inventory.sql`、Ent Schema 字段和生成
  代码。发布前必须人工审核 Migration、确认生产备份方案并通过数据库边界门禁，开发
  和测试过程不得连接或修改生产数据库。
...
官方支付 Service、管理 Handler 和套餐页面仅保留事务生命周期委托、DTO 与组件接入。
