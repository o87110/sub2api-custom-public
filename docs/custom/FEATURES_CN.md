...
管理员还可在订阅套餐创建/编辑弹窗中开启“允许批量重置配额”。开关默认关闭并即时
作用于已有有效订阅，与套餐是否在售或是否售罄无关。订阅管理页的“批量重置配额”
弹窗会先列出候选用户、套餐、本周期累计和已重置次数，默认全选，并允许逐项取消：

- 只纳入未删除、`active`、已开始且未到期的订阅；
- 当前实际生效周期必须是 `payment` 来源，且能通过支付订单准确追溯到仍存在并开启
  开关的套餐；订单用户、订单订阅分组、套餐分组必须与当前订阅一致；
- 提前续订只认当前周期套餐，周期边界后才改用待生效套餐；`legacy`、兑换码、管理员
  分配及其他无法准确归属套餐的周期安全排除，不按分组或最近订单猜测；
- 提交接口要求 `Idempotency-Key`，后端对订阅 ID 去重并逐项重新检查资格。每项独立
  执行，状态变化记为跳过，实际重置错误记为失败，不因单项失败回滚其他成功项；
...
资格字段。发布前必须人工审核 Migration、备份 `user_subscriptions`、
