...
- 新建与编辑覆盖留空不限量、正整数提交、`0`/负数/小数拒绝及三态更新；售罄 `0`
  未修改而保存其他字段时不回传数量；
- 表格覆盖“无限制”、正整数、“0 / 已售罄”和自动下架标识；售罄套餐不能直接上架，
  补货和改为不限量遵守自动/手动下架恢复差异；
- 后端覆盖不限量兼容、最后一份自动下架、并发预占不超卖、取消/超时/支付创建失败
  释放且幂等、履约消费幂等、延迟支付重新预占与补货重试，以及退款不归还；
