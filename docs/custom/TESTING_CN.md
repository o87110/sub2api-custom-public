...
- 外部 Key、失效 Key、解密失败、无分组或平台不匹配时返回 `null` 并显示备用标签；
- 批量查询失败时监控列表仍成功，可用性、延迟和时间线保持原行为；
- 自动倍率存在时不重复显示旧手填倍率，倍率缺失时保留原 `group_name`；
- 倍率标签在亮色、暗色和窄屏下可读，使用等宽数字且包含可访问说明；
- 响应、日志和前端测试输出不包含 API Key；
- Candidate Tree 数据库边界检查确认无 Migration、Schema、实体字段或 SQL 结构变化。
