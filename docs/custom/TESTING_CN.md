...
- 候选查询只返回有效且当前周期可由支付订单精确追溯到已开启套餐的订阅；覆盖套餐
  下架/售罄仍可入选、关闭开关、过期、暂停、撤销、`legacy`、兑换码、管理员分配、
  订单用户/分组不一致及套餐分组不一致的排除行为；
- 第 25 天提前续订仍使用当前套餐资格，第 30 天边界后才使用待生效套餐；边界时执行
  批量重置必须先推进周期，再把重置次数记为新周期的第一次；
- 批量提交覆盖 ID 去重、提交前资格变化为跳过、单项失败不阻断其余项、日周月统一
  重置，以及同一幂等键重放不重复增加次数；后端和前端均拒绝超过 300 个订阅，300 条
...
- 前端覆盖候选加载、空状态、默认全选、半选、取消勾选、零选择禁用、仅提交所选项、
