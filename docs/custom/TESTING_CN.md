...
- 旧配置缺少 `api_audit_ban_enabled` 与 `api_audit_ban_threshold` 时，规则关闭且阈值继承
  当前默认封禁阈值；PUT 省略字段保持原值，阈值超出 `1-1000` 时拒绝保存；
- API 专属 SQL 只统计窗口内、最后一次自动封禁后、`flagged=true` 且 action 为
  `allow`、`block` 或范围内 `cyber_policy` 的记录；关键词、hash、范围外、错误及未命中均排除；
- 验证默认规则单独达到、API 规则单独提前达到和两者同时达到；同时达到时只更新用户、
  刷新认证缓存并发送一次封禁通知；
- 验证自动封禁总开关、API 专属开关、管理员豁免和用户专属默认阈值的组合；用户专属值
  不覆盖 API 专属阈值；
- 保存配置不立即扫描或封禁，下一次 API 审计或范围内 `cyber_policy` 命中把当前事件加一并读取累计窗口内历史；
- 前端验证旧响应回退、开关与禁用态、数值保留、`1-1000` 行内错误、保存失败聚焦、
  自动封禁关闭提示以及中英文“额外规则只能收紧”说明；
- 本功能不新增 Migration、Schema、实体字段或 SQL 表字段。
...
- 充值和订阅默认选择易支付，点击官方后载荷正确；失败不自动跨渠道重试，只显示
  手动备用提示；
- 支付订单生命周期必须覆盖用户/管理员取消、超时过期、库存释放和并发竞态；上游
  查询失败、配置不可用或关闭请求失败时，取消/过期应本地降级成功并记录审计，明确
  已支付状态仍拒绝取消，迟到支付恢复窗口保持现有规则；
...
- 新建分组、旧分组和缺少 `blocked_models` 的鉴权快照默认不禁用任何模型；配置归一化
  去空、去重并保持精确匹配；
...
- 普通、Codex、Gemini 和批量图片模型列表验证直接禁用及“所有映射路径均禁用”隐藏，
  同时保留仍有允许最终路径的公开模型；
...
- 鉴权缓存 JSON 往返保留黑名单，v19 快照被淘汰；前向 Migration 221 覆盖已发布 193 的
  旧列 Trigger，Migration 238 在官方 Migration 235/236 更名后把函数绑定到
  `model_allowlist`；直改当前白名单必须入队且无实际变化不得入队，并验证分组更新和删除
  不再引用旧列；数据库边界检查必须确认 238 不新增表、列、Schema 字段或数据回写，
  发布前仍需人工审核 Migration。
...
- 同一邀请人多订单、多邀请人同批、混合冻结/可用/余额来源的分配与前后快照准确；
- 预览 token 绑定返利流水、返利账户和用户资金更新时间，期间发生返利转余额、计费扣款
  或其他资金变化时执行返回过期冲突且不写任何数据；
- 任一目标不存在、重复、已撤销或资金汇总异常时整批回滚；成功时原返利行只把
  `action` 改为 `reversed` 并清空冻结时间，金额、来源、订单、创建时间和
  `AFFILIATE_REBATE_APPLIED` 保持不变；相同幂等键可恢复已提交结果，不同幂等键不得
  二次扣减；
- 撤销后懒解冻、用户累计返利、已返利人数及单人上限排除原返利；余额缓存和认证缓存
  失效，负余额立即阻止余额模式，订阅模式保持原资格；
- 首次撤销提交后缓存失效失败、请求取消以及通用幂等记录直接 replay 时，使用相同幂等键
  重试会重新查询已提交撤销记录并再次失效认证与余额缓存；缓存操作使用独立超时上下文；
- 管理端默认可查看有效与已撤销记录，历史手工撤销显示快照不可用；用户端不显示已撤销
  金额；
- 前端验证当前页选择、全选忽略已撤销行、筛选/分页/刷新清空选择、服务端预览、必填
  原因、负余额额外确认、稳定幂等键重试、成功反馈和失败保留选择；
- Migration 231 必须在 PostgreSQL 集成测试中验证约束、索引、通用历史回填与重复执行，
  发布前人工审核并备份 `user_affiliate_ledger`、`user_affiliates`、`users`、
  `payment_audit_logs`、`payment_orders` 和 `schema_migrations`。
