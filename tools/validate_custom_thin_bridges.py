...
APPROVED_DELEGATE_VIEW_CALL_DELTAS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    'backend/internal/service/idempotency.go': _approved_call_deltas(
        ('Execute', {
            'Add': 1,
            'c.renewProcessingLease': 1,
            'cancel': 2,
            'context.WithTimeout': 2,
            'context.WithoutCancel': 2,
            'idempotencyexecution.New': 1,
    ...
    'backend/internal/service/subscription_service.go': _approved_call_deltas(
        ...
        ('ExtendSubscription', {
            ...
            's.invalidateAdjustedSubscriptionCache': 1,
            's.now': 1,
            's.userSubRepo.GetByIDForUpdate': 1,
            's.withSubscriptionUpdateTx': 1,
        }),
        ('FinalizeSubscriptionRefundDeduction', {
            'errors.Is': 1,
            'infraerrors.InternalServer': 1,
    ...
    'frontend/src/views/admin/SubscriptionsView.vue': _approved_call_deltas(
        ...
        ('<template:@change>', {'resetAssignUsers': 1}),
        ('<template:@click>', {'clearSelection': 1}),
        ('<template:@close>', {'bulkAction = null': 1}),
        ('<template:@completed>', {'handleBulkCompleted': 1}),
        ('<template:@update:selected-keys>', {'handleSelectedKeysUpdate': 1}),
        ('<top-level>', {'assignUsers.filter': 1, 'assignUsers.some': 1, 'openBulkAction': 1, 'ref': 1, 't': 12}),
        ('applyFilters', {'clearSelection': 1}),
        ('debounceSearchUsers', {'userSearchKeyword.value.trim': 1}),
        ('handleAssignSubscription', {
            'Number.isInteger': 1,
            'Set': 1,
            'adminAPI.subscriptions.bulkAssign': 1,
            'appStore.showSuccess': 1,
            'assignUsers.value.filter': 1,
            'assignUsers.value.map': 1,
            'loadSubscriptions': 1,
            'result.subscriptions.map': 1,
            'successIds.has': 1,
            't': 1,
        }),
        ('handlePageChange', {'clearSelection': 1}),
        ...
        ('loadSubscriptions', {
            ...
            'selectedIds.value.filter': 1,
            'setSelectedIds': 1,
            'visibleIds.has': 1,
        }),
        ('searchUsers', {'adminAPI.users.list': 1}),
        ('selectUser', {'assignUsers.value.some': 1}),
        ('handleResetQuota', {'Date.now': 1, 'Math.random': 1, 'randomUUID': 1}),
    ),
    'frontend/src/views/user/KeysView.vue': _approved_call_deltas(
        ('<top-level>', {
            'KEY_GROUP_PROVIDERS.map': 1,
            'apiKeys.value.filter': 1,
            'balanceRequirementForGroup': 2,
            'balanceRequirementsByGroupID.value.get': 1,
            'computed': 3,
            'customApiKeyBulkText': 1,
            'formGroupOptions.value.some': 1,
            'getKeyGroupProvider': 2,
            'groupBalanceRequirement': 1,
            'groupBalanceRequirementsByID': 1,
            'groupOptions.value.filter': 1,
            'groups.value.filter': 1,
            'platformBadgeLightClass': 1,
            'providers.find': 1,
            'providers.some': 1,
            'ref': 1,
            'selectCreateProvider': 2,
            'selectedIds.value.includes': 1,
            't': 9,
            'watch': 1,
        }),
        ('<template:@click>', {'selectedIds = []': 1, 'showBulkEditModal = true': 1}),
        ('<template:@close>', {'showBulkEditModal = false': 1}),
        ('<template:@updated>', {'handleBulkUpdated': 1}),
...
APPROVED_DELEGATE_VIEW_CONTROL: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    "backend/internal/service/idempotency.go": (
        ("Execute", "if err != nil {"),
        ("Execute", "if opts.ExecutionTimeout > 0 {"),
        ("Execute", "if opts.ExecutionTimeout > 0 {"),
        ("Execute", "if opts.ExecutionTimeout > 0 {"),
        ("Execute", "if markErr := c.repo.MarkFailedRetryable(persistCtx, record.ID, reason, backoffUntil, expiresAt); markErr != nil {"),
        ("Execute", "if markErr := c.repo.MarkSucceeded(persistCtx, record.ID, 200, storedBody, expiresAt); markErr != nil {"),
    ),
    ...
    "backend/internal/service/admin_group.go": (
        ("compositeDefaultModelsListCandidateIDs", "for _, platform := range []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo} {"),
    ...
    "backend/internal/service/subscription_service.go": (
        ...
        ("ExtendSubscription", "if err != nil {"),
        ("ExtendSubscription", "if !newExpiresAt.After(now) {"),
        ("ExtendSubscription", "if days < -MaxValidityDays {"),
        ("ExtendSubscription", "if days > MaxValidityDays {"),
        ("ExtendSubscription", "if err := s.userSubRepo.ExtendExpiry(txCtx, subscriptionID, newExpiresAt); err != nil {"),
        ("ExtendSubscription", "if err := s.userSubRepo.UpdateStatus(txCtx, subscriptionID, SubscriptionStatusActive); err != nil {"),
        ("ExtendSubscription", "if isExpired && days < 0 {"),
        ("ExtendSubscription", "if isExpired {"),
        ("ExtendSubscription", "if newExpiresAt.After(MaxExpiresAt) {"),
        ("ExtendSubscription", "if s.now != nil {"),
        ("ExtendSubscription", "if sub.Status == SubscriptionStatusExpired {"),
    ...
    "frontend/src/views/admin/SubscriptionsView.vue": (
        ("<top-level>", '<div v-if="batchAssignEnabled && assignUsers.length > 0" class="mt-2 space-y-2" data-test="assign-users">'),
        ("<top-level>", '<div v-if="batchAssignResult" class="space-y-2 text-sm" role="status" data-test="batch-assign-result">'),
        ("<top-level>", '<li v-for="(error, index) in batchAssignResult.errors" :key="index">{{ error }}</li>'),
        ("<top-level>", '<li v-for="user in assignUsers" :key="user.id" class="flex items-center justify-between gap-2 rounded-lg bg-gray-50 px-3 py-1 text-sm dark:bg-dark-700">'),
        ("<top-level>", '<p v-if="batchAssignEnabled" class="input-hint">{{ t(\'admin.subscriptions.batchAssign.hint\') }}</p>'),
        ("<top-level>", '<p v-if="batchAssignResult.failed_count > 0" class="input-hint">{{ t(\'admin.subscriptions.batchAssign.retryHint\') }}</p>'),
        ("<top-level>", '<ul v-if="batchAssignResult.errors.length" class="max-h-40 space-y-1 overflow-y-auto text-red-600 dark:text-red-400">'),
        ("<top-level>", 'v-for="action in bulkActions"'),
        ("<top-level>", 'v-if="bulkAction !== null"'),
        ("<top-level>", 'v-if="selectedCount > 0"'),
        ("closeAssignModal", "if (submitting.value) return"),
        ("debounceSearchUsers", "if (selectedUser.value && userSearchKeyword.value.trim() !== selectedUser.value.email) {"),
        ("handleAssignSubscription", "if (!Number.isInteger(assignForm.validity_days) || assignForm.validity_days < 1 || assignForm.validity_days > 36500) {"),
        ("handleAssignSubscription", "if (batchAssignEnabled.value ? assignUsers.value.length === 0 : !assignForm.user_id) {"),
        ("handleAssignSubscription", "if (batchAssignEnabled.value) {"),
        ("handleAssignSubscription", "if (result.success_count > 0) {"),
        ("handleAssignSubscription", "if (submitting.value) return"),
        ("selectUser", "if (assignUsers.value.length < 100 && !assignUsers.value.some((selected) => selected.id === user.id)) {"),
        ("selectUser", "if (batchAssignEnabled.value) {"),
        ("selectUser", "if (submitting.value) return"),
    ),
    ...
    "frontend/src/views/user/KeysView.vue": (
        ("<top-level>", '<div v-if="selectedIds.length" class="flex flex-wrap items-center gap-3 text-sm">'),
        ("<top-level>", '<fieldset v-if="!showEditModal" data-tour="key-form-provider">'),
        ("<top-level>", 'v-for="platform in KEY_GROUP_PROVIDER_ICONS[provider.value]"'),
        ("<top-level>", 'v-for="provider in createProviderOptions"'),
        ("<top-level>", 'v-if="createProvider === provider.value"'),
        ("<top-level>", "if (!formGroupOptions.value.some((group) => group.value === formData.value.group_id)) {"),
        ("<top-level>", "if (!isOpen) return"),
        ("<top-level>", "if (!wasOpen || !providers.some((provider) => provider.value === createProvider.value && provider.count > 0)) {"),
        ("resetQuotaUsed", "if (!key) return"),
        ("resetQuotaUsed", "if (key.status !== updatedKey.status) {"),
        ("resetQuotaUsed", "if (selectedKey.value?.id === key.id) {"),
    ...
    "frontend/src/views/user/PaymentView.vue": (
        ("<top-level>", '<div v-if="tabs.length === 0" class="card py-16 text-center">'),
        ("<top-level>", '<template v-else-if="activeTab === \'recharge\'">'),
        ("<top-level>", "if (available.some((tab) => tab.key === activeTab.value)) return"),
        ("<top-level>", "if (checkout.value.balance_disabled && subscriptionEnabled.value) {"),
        ("<top-level>", "if (leavingSubscription) selectedPlan.value = null"),
        ("<top-level>", "if (route.query.tab === 'subscription' && subscriptionEnabled.value) {"),
        ("<top-level>", "if (subscriptionEnabled.value) {"),
        ("tabs", "if (subscriptionEnabled.value) result.push({ key: 'subscription', label: t('payment.tabSubscribe') })"),
...
APPROVED_UPGRADE_NEW_BRIDGE_FUNCTIONS: dict[str, frozenset[str]] = {
    ...
    "backend/internal/service/idempotency.go": frozenset({
        "renewProcessingLease",
    }),
    "frontend/src/views/admin/SubscriptionsView.vue": frozenset({
        "bulkTargets",
        "handleBulkCompleted",
        "handleSelectedKeysUpdate",
        "openBulkAction",
...
APPROVED_DELEGATE_VIEW_CALL_DELTAS.update({
    ...
    "backend/internal/service/openai_images_responses.go": _approved_call_deltas(
        ("forwardOpenAIImagesOAuth", {
            "SetOpsUpstreamModel": 1,
            "account.GetMappedModel": 1,
            "beginUpstreamResponseModelObservation": 1,
            "buildOpenAIImagesOAuthPayload": 1,
            "isOpenAIImagesForceResponses": 1,
            "isOpenAIImagesMainModelError": 1,
            "observedUpstreamResponseModel": 2,
            "observedUpstreamResponseModelConflict": 2,
            "s.forwardOpenAIImagesOAuth": 1,
            "s.handleCodexDirectImagesNonStreamingResponse": 1,
            "s.handleOpenAIImagesErrorResponse": 1,
            "s.handleOpenAIImagesStreamingResponse": 1,
            "upstreamReq.Header.Del": 1,
            "upstreamReq.Header.Set": 2,
            "url.Parse": 1,
            "usesCodexDirectImages": 1,
            "validateOpenAIImagesModel": 1,
            "withOpenAIImagesForceResponses": 1,
            "enforceResolvedModelAccess": 1,
        }),
        ("handleOpenAIImagesErrorResponse", {
            "isOpenAIImagesMainModelError": 1,
        }),
        ("handleOpenAIImagesOAuthResponseError", {
            "isOpenAIImagesMainModelError": 1,
            "openAIImagesUpstreamErrorResponseBody": 1,
            "writeOpenAIImagesUpstreamErrorResponse": 1,
        }),
        ("openAIImagesToolUsageFromGJSON", {"min": 1}),
    ),
...
APPROVED_DELEGATE_VIEW_CONTROL.update({
    ...
    "backend/internal/service/openai_images_responses.go": (
        ("forwardOpenAIImagesOAuth", "if !direct && isOpenAIImagesMainModelError(resp.StatusCode, respBody) {"),
        ("forwardOpenAIImagesOAuth", "if !parsed.Stream {"),
        ("forwardOpenAIImagesOAuth", "if direct && (resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusMethodNotAllowed) {"),
        ("forwardOpenAIImagesOAuth", "if direct {"),
        ("forwardOpenAIImagesOAuth", "if direct {"),
        ("forwardOpenAIImagesOAuth", "if direct {"),
        ("forwardOpenAIImagesOAuth", "if direct {"),
        ("forwardOpenAIImagesOAuth", "if err != nil {"),
        ("forwardOpenAIImagesOAuth", "if err := validateOpenAIImagesModel(upstreamModel); err != nil {"),
        ("handleOpenAIImagesErrorResponse", "if account.IsOpenAIOAuthLike() && isOpenAIImagesMainModelError(resp.StatusCode, body) {"),
        ("handleOpenAIImagesOAuthResponseError", "if !responseWritten {"),
        ("handleOpenAIImagesOAuthResponseError", "if isOpenAIImagesMainModelError(upstreamErr.StatusCode, openAIImagesUpstreamErrorResponseBody(upstreamErr)) {"),
