export default {
  ...
  subscriptionProgress: {
    ...
    weekly: '每周',
    monthly: '每月',
    cycleStats: '本周期累计 ${usage} · 已重置 {count} 次',
  ...
  payment: {
    ...
    channels: {
      ...
      officialAlipay: '官方渠道支付宝',
      easypayWxpay: '易支付渠道微信',
    ...
    errors: {
      ...
      switchChannelHint: '可手动切换至“{channel}”后重试，系统不会自动创建备用渠道订单。',
      ...
      PLAN_RENEWAL_GRACE_INVALID: '过期宽限天数必须是 0 到 30 之间的整数',
      ...
      NOT_FOUND: '订单不存在',
      FORBIDDEN: '无权限操作此订单',
      ORDER_ALREADY_PAID: '订单已支付，不能取消，请等待订单完成',
      PAYMENT_STATUS_UNAVAILABLE: '暂时无法确认支付渠道状态，请稍后重试',
      ORDER_STATUS_CHANGED: '订单状态已变化，请刷新后重试',
    ...
    admin: {
      ...
      remainingQuantitySoldOutHint: '当前已售罄；请输入正整数补货，或清空改为不限量',
      remainingQuantityInvalid: '剩余可买数量只能留空或填写正整数',
      remainingQuantityInvalidAllowZero: '剩余可买数量只能留空或填写非负整数',
      inventoryUnlimited: '无限制',
      inventorySoldOut: '0 / 已售罄',
      inventoryAutoDelisted: '自动下架',
      inventoryPurchaseDisabled: '禁用购买',
      soldOutAction: '售罄后处理',
      soldOutActionDelist: '自动下架',
      soldOutActionDisablePurchase: '保留展示并禁用购买',
      soldOutActionHint: '默认自动下架；保留展示时新用户不能购买，符合套餐续费策略的老用户仍可续费',
      soldOutCannotList: '自动下架模式的套餐已售罄，请先补货、改为不限量或切换为禁用购买后再上架',
      renewalPolicy: '老用户续费',
      allowExistingUserRenewal: '允许老用户续费',
      allowExistingUserRenewalHint: '开启后，已有同分组有效订阅或仍在过期宽限期内的用户，可以续费下架或售罄套餐；续费不占用剩余可买数量。',
      renewalGraceDays: '过期宽限天数',
      renewalGraceDaysHint: '支持 0–30 天；0 表示订阅过期后不再允许续费。有效订阅不受此天数限制。',
      renewalGraceDaysInvalid: '过期宽限天数必须是 0 到 30 之间的整数',
      renewalEnabled: '允许续费',
      renewalDisabled: '未开启',
      renewalGraceDaysValue: '过期后 {days} 天内',
      onSale: '上架',
      offSale: '下架',
      group: '分组',
      groupId: '分组 ID',
