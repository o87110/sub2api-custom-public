export default {
    ...
    affiliates: {
      ...
      reversal: {
        ...
        selectedAmount: '返利合计 ${amount}',
        clearSelection: '清除选择',
        selectOrder: '选择订单 #{order}',
        statusColumn: '返利状态',
        statusFilter: '返利状态筛选',
        statusAll: '全部返利',
        statusActive: '有效',
        statusReversed: '已撤销',
        deductionSummary: '冻结 ${frozen} / 可用 ${quota} / 余额 ${balance}',
        snapshotUnavailable: '历史手工撤销，资金快照不可用',
        reversedBy: '操作管理员 #{user}',
        dialogTitle: '确认撤销返利',
        previewLoading: '正在核算冻结额度、可用返利和余额影响…',
        previewFailed: '无法生成返利撤销预览',
        orderCount: '订单数',
        totalAmount: '撤销返利总额',
        frozenDeducted: '扣冻结返利',
        quotaDeducted: '扣可用返利',
        balanceDeducted: '扣用户余额',
        balanceAfter: '撤销后余额',
        negativeWarning: '此次撤销将使 {count} 位邀请人的余额变为负数',
        negativeHint: '负余额会阻止后续余额计费请求，但不会取消仍有效的订阅。后续充值将优先抵消负余额。',
        confirmNegative: '我确认允许将上述邀请人的余额扣为负数',
        reasonLabel: '撤销原因',
        reasonPlaceholder: '请填写可供审计追溯的撤销原因',
        irreversibleHint: '撤销后不可恢复，原订单和支付记录保持不变。',
        confirmAction: '确认完整撤销',
        submitting: '正在撤销…',
        submitFailed: '返利撤销失败，未产生部分成功',
        success: '已完整撤销 {count} 个订单的返利，合计 ${amount}',
        errors: {
          AFFILIATE_REVERSAL_INVALID_ORDERS: '请选择 1–100 个有效返利订单。',
          AFFILIATE_REVERSAL_INVALID_REASON: '请填写撤销原因，最多 500 个字符。',
          AFFILIATE_REVERSAL_TARGET_INVALID: '所选返利已变化、已撤销或不再存在，请刷新列表后重试。',
          AFFILIATE_REVERSAL_PREVIEW_STALE: '资金状态已变化，请重新生成撤销预览。',
          AFFILIATE_REVERSAL_NEGATIVE_BALANCE_CONFIRM_REQUIRED: '此次撤销会产生负余额，请先勾选风险确认。',
          AFFILIATE_REVERSAL_INVARIANT: '返利资金汇总不一致，系统已整批回滚，请人工核查。'
        }
      }
    },
    ...
    groups: {
      ...
      form: {
        ...
        minimumBalance: '最低可用余额门槛',
        minimumBalanceHint: '用户余额必须严格高于此金额才能绑定或使用该分组；余额等于门槛时可用额度为 0。设为 0 表示不启用。',
        minimumBalanceInvalid: '最低可用余额门槛必须是大于等于 0 的有效金额',
        rpmLimit: '每分钟请求数 (RPM)',
        rpmLimitPlaceholder: '0 表示不限制',
      ...
      modelAllowlist: {
        emptySelectionError: '启用模型白名单后至少选择一个模型'
      },
      codexModelsManifest: {
        title: 'Codex 模型清单',
