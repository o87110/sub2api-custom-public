export default {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        groupName: '分组名称',
        groupNamePlaceholder: '用户监控卡片上的自定义分组标签（可选）',
        groupNameHint: '显示在用户监控卡片的模型后方，也可用于管理后台搜索；不会被倍率覆盖。',
        groupRateOverride: '手动倍率覆盖',
    ...
    subscriptions: {
      ...
      manualEligibility: {
        ...
        actionEnabled: '资格已开',
        actionDisabled: '资格未开',
        assignLabel: '允许批量重置配额',
        assignHint: '仅控制本次手动分配产生的订阅周期；默认关闭，后续可在订阅操作区单独修改。',
        title: '修改批量重置资格',
        enable: '开启资格',
        disable: '关闭资格',
        enableConfirm: "确定为 '{user}' 的当前手动订阅周期开启批量重置资格吗？不会重置用量或改变有效期。",
        disableConfirm: "确定关闭 '{user}' 当前手动订阅周期的批量重置资格吗？不会重置用量或改变有效期。",
        enabledSuccess: '已开启当前周期的批量重置资格',
        disabledSuccess: '已关闭当前周期的批量重置资格',
        updateFailed: '更新批量重置资格失败'
      },
      bulkReset: {
        action: '批量重置配额',
        title: '批量重置用量配额',
        loading: '正在加载符合条件的订阅…',
        loadFailed: '候选订阅加载失败，请重试。',
        retry: '重新加载',
        summary: '共 {users} 位用户、{subscriptions} 个有效订阅',
        selected: '已选择 {count} 个订阅',
        selectionLimit: '为保证结果可安全重放，每批最多选择 {count} 个订阅；其余候选可在本批完成后继续重置。',
        selectAll: '选择全部候选订阅',
        selectSubscription: '选择 {email} 的 {source} 订阅',
        user: '用户',
        source: '订阅来源',
        manualSource: '手动分配 · {group}',
        cycleUsage: '本周期累计',
        resetCount: '已重置次数',
        warning: '确认后将同时清空所选订阅的日、周、月当前用量；本周期累计用量和历史日志会保留。',
        submit: '重置所选 {count} 个订阅',
        submitting: '正在重置…',
        submitFailed: '未能确认本次批量操作结果。候选选择已锁定，可使用同一幂等键安全重试。',
        retrySubmit: '重试本次重置',
        emptyTitle: '当前没有可批量重置的订阅',
        emptyHint: '支付订阅需在套餐编辑中开启“允许批量重置配额”；手动分配或历史有效订阅需为当前周期单独开启资格。',
        requested: '提交',
        success: '成功',
        skipped: '跳过',
        failed: '失败',
        status: '结果',
        reason: '说明',
        completedReason: '日、周、月当前用量已重置',
        noLongerEligible: '订阅状态、来源或批量重置资格已变化，本次已安全跳过',
        resetFailedReason: '重置执行失败，请稍后重试',
        requestFailedReason: '批量请求失败，请稍后重试'
      },
