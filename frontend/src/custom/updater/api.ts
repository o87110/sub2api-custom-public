...
export interface VersionInfo {
  ...
  has_official_update?: boolean
  official_release_info?: ReleaseInfo
  official_release_warning?: string
}

/**
 * Get current version
 */
export async function getVersion(): Promise<{ version: string; build_version: string }> {
  const { data } = await apiClient.get<{ version: string; build_version: string }>(
