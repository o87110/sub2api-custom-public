  ...
  <div class="relative">
    ...
    <template v-if="isAdmin">
      ...
      >
        ...
        ></span>
        <!-- Update indicator -->
        <span v-if="versionCheckError" class="relative flex h-2 w-2">
      ...
      <transition name="dropdown">
        <div
          v-if="dropdownOpen"
          ref="dropdownRef"
          class="fixed inset-x-4 top-20 z-50 max-h-[calc(100vh-6rem)] overflow-y-auto whitespace-normal rounded-xl border border-gray-200 bg-white shadow-lg transition-all duration-200 dark:border-dark-700 dark:bg-dark-800 lg:absolute lg:inset-x-auto lg:left-0 lg:top-auto lg:mt-2 lg:max-h-none lg:w-[350px] lg:max-w-[calc(100vw-2rem)] lg:overflow-hidden"
        >
            ...
            <template v-else>
              ...
              <div class="mb-4 text-center">
                <div class="inline-flex items-center gap-2">
                  <span
                    v-if="currentBuildVersion"
                    class="text-2xl font-bold text-gray-900 dark:text-white"
                    >v{{ currentBuildVersion }}</span
                  >
                  <span v-else class="text-2xl font-bold text-gray-400 dark:text-dark-500">--</span>
                  <!-- Show check mark when up to date -->
                  <span
                    v-if="!hasUpdate && !versionCheckError"
              ...
              <!-- Release sources -->
              <div
                class="mb-4 overflow-hidden rounded-lg border border-gray-100 dark:border-dark-700"
              >
                <div
                  class="flex items-center justify-between gap-3 px-3 py-2"
                  :title="updateRepository || undefined"
                >
                  <div class="min-w-0">
                    <p class="text-[11px] font-medium text-gray-400 dark:text-dark-500">
                      {{ customT('customRelease') }}
                    </p>
                    <p class="truncate text-xs font-semibold text-gray-700 dark:text-dark-200">
                      {{ sourceLatestVersion ? 'v' + sourceLatestVersion : '--' }}
                    </p>
                  </div>
                  <a
                    ...
                    class="flex shrink-0 items-center gap-1 whitespace-nowrap text-xs text-gray-500 transition-colors hover:text-gray-700 dark:text-dark-400 dark:hover:text-dark-200"
                  >
                    {{ customT('viewCustomRelease') }}
                    <Icon name="externalLink" size="xs" :stroke-width="2" />
                  </a>
                </div>

                <div
                  class="flex items-center justify-between gap-3 border-t border-gray-100 px-3 py-2 dark:border-dark-700"
                  :title="officialReleaseWarning || officialRepository || undefined"
                >
                  <div class="min-w-0">
                    <p class="text-[11px] font-medium text-gray-400 dark:text-dark-500">
                      {{ customT('officialRelease') }}
                    </p>
                    <p
                      v-if="officialLatestVersion"
                      class="flex min-w-0 items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-dark-200"
                    >
                      <span class="truncate">v{{ officialLatestVersion }}</span>
                      <span
                        v-if="hasOfficialUpdate"
                      ...
                      >
                        {{ customT('officialUpdateAvailable') }}
                      </span>
                    </p>
                    <p v-else class="truncate text-xs text-gray-400 dark:text-dark-500">
                      {{
                        officialReleaseWarning ? customT('officialReleaseUnavailable') : '--'
                      }}
                    </p>
                  </div>
                  <a
              ...
              <div v-if="displayError" class="space-y-2">
                <div
                    ...
                    <div v-if="rollbackPanelOpen" class="mt-2 space-y-2">
                      ...
                      <template v-else>
                        ...
                        <div
                          ...
                          :key="item.version"
                          class="flex w-full items-stretch rounded-lg border transition-all"
                          :class="
                        ...
                        >
                          <button
                            ...
                            class="flex min-w-0 flex-1 items-start gap-2 px-3 py-2 text-left disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            <span
                              class="mt-1 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full border transition-colors"
                              :class="
                                selectedRollbackVersion === item.version
                                  ? 'border-amber-500'
                            ...
                            <span
                              ...
                              "
                            >
                              <span class="block">v{{ item.version }}</span>
                              <span
                                v-if="item.name"
                                class="block truncate text-[11px] font-normal text-gray-500 dark:text-dark-400"
                              >
                                {{ item.name }}
                              </span>
                              <span
                                v-if="summarizeReleaseNotes(item.body)"
                                class="rollback-release-notes mt-0.5 text-[10px] font-normal leading-4 text-gray-400 dark:text-dark-500"
                              >
                                {{ summarizeReleaseNotes(item.body) }}
                              </span>
                            </span>
                            <span
                              class="shrink-0 pt-0.5 text-[11px] tabular-nums text-gray-400 dark:text-dark-500"
                            >
                              {{ formatPublishedAt(item.published_at) }}
                            </span>
                          </button>
                          <a
                            v-if="item.html_url && item.html_url !== '#'"
                            :href="item.html_url"
                            target="_blank"
                            rel="noopener noreferrer"
                            class="flex shrink-0 items-center border-l border-gray-200 px-2 text-gray-400 transition-colors hover:text-gray-600 dark:border-dark-700 dark:text-dark-500 dark:hover:text-dark-300"
                            :title="t('version.viewChangelog')"
                            :aria-label="t('version.viewChangelog')"
                          >
                            <Icon name="externalLink" size="xs" :stroke-width="2" />
                          </a>
                        </div>

                        <!-- Selected version: authenticated backend rollback -->
                        <transition name="rollback">
...
import { customUpdaterText, type CustomUpdaterTextKey } from '@/custom/updater/i18n'

const { t, locale } = useI18n()
const customT = (key: CustomUpdaterTextKey) => customUpdaterText(locale.value, key)
...
const updaterStore = useCustomUpdaterStore()
...
const loading = computed(() => updaterStore.versionLoading)
const versionCheckError = computed(() => updaterStore.versionError)
const currentVersion = computed(() => updaterStore.currentVersion || props.version || '')
const currentBuildVersion = computed(() => updaterStore.currentBuildVersion || currentVersion.value)
const latestVersion = computed(() => updaterStore.latestVersion)
const hasUpdate = computed(() => updaterStore.hasUpdate)
const releaseInfo = computed(() => updaterStore.releaseInfo)
const buildType = computed(() => updaterStore.buildType)
const updateRepository = computed(() => updaterStore.updateRepository)
const officialRepository = computed(() => updaterStore.officialRepository)
const officialLatestVersion = computed(() => updaterStore.officialLatestVersion)
const hasOfficialUpdate = computed(() => updaterStore.hasOfficialUpdate)
const officialReleaseInfo = computed(() => updaterStore.officialReleaseInfo)
const officialReleaseWarning = computed(() => updaterStore.officialReleaseWarning)
const sourceLatestVersion = computed(
  () => latestVersion.value || currentBuildVersion.value || currentVersion.value
)
...
const displayError = computed(() => updateError.value || versionCheckError.value)
const isVersionCheckFailure = computed(
  () => !updateError.value && Boolean(versionCheckError.value)
)
...
async function refreshVersion(force = true) {
  ...
  await updaterStore.fetchVersion(force)
}
...
async function handleUpdate() {
  ...
  try {
    ...
    updaterStore.clearVersionCache()
  } catch (error: unknown) {
...
async function retryAfterError() {
  if (isVersionCheckFailure.value) {
...
onMounted(() => {
  if (isAdmin.value) {
    ...
    updaterStore.fetchVersion(false)
  }
