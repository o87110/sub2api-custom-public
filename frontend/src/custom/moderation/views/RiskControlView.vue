                ...
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-dark-700/60">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <div
                        v-if="row.action === 'cyber_policy_out_of_scope'"
                        class="text-xs text-gray-400"
                      >
                        {{ customT('autoBanNo') }} /
                        {{ customT('notificationNotSent') }}
                      </div>
        ...
        <div v-if="inputDetailRow" class="space-y-5">
          ...
          <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-800">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ customT(inputDetailContentLabelKey) }}</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
...
const customT = (key: CustomModerationTextKey) => customModerationText(locale.value, key)
...
const inputDetailContentLabelKey = computed<CustomModerationTextKey>(() => (
  inputDetailRow.value?.matched_keyword
  && inputDetailRow.value.input_excerpt.includes(keywordContextSeparator)
...
function resultLabel(row: ContentModerationLog): string {
  if (row.action === 'cyber_policy_out_of_scope') return customT('cyberPolicyOutOfScope')
...
function resultBadgeClass(row: ContentModerationLog): string {
  ...
  if (row.action === 'cyber_policy_out_of_scope') return 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'
...
function violationCountText(row: ContentModerationLog): string {
  ...
  if (row.action === 'cyber_policy_out_of_scope') return customT('violationNotCounted')
  if (row.violation_count === 0) return customT('violationNotCounted')
