...
const messages: Record<'zh' | 'en', Record<CustomModerationTextKey, string>> = {
  zh: {
    ...
    apiAuditBanHint: 'API 审计与范围内 cyber_policy 命中仍计入上方总累计，并额外进入专属累计；任一达到阈值即封号，因此该规则只能收紧封禁条件。',
    apiAuditBanThreshold: 'API 审计专属封禁次数',
    apiAuditBanActiveHint: 'API 审计或范围内 cyber_policy 专属累计达到该次数时执行自动封禁。',
  ...
  en: {
    ...
    apiAuditBanHint: 'API moderation and in-scope cyber_policy hits still count toward the total above and also enter this dedicated count. Either threshold can ban, so this rule can only make enforcement stricter.',
    apiAuditBanThreshold: 'API moderation ban trigger count',
    apiAuditBanActiveHint: 'Automatic banning runs when the dedicated API moderation or in-scope cyber_policy count reaches this value.',
