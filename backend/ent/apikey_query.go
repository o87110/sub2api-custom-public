...
type APIKeyQuery struct {
	...
	withGroupLinks *APIKeyGroupQuery
...
func (_q *APIKeyQuery) QueryGroupLinks() *APIKeyGroupQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, apikey.GroupLinksTable, apikey.GroupLinksColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withGroupLinks: _q.withGroupLinks.Clone(),
...
func (_q *APIKeyQuery) WithGroupLinks(opts ...func(*APIKeyGroupQuery)) *APIKeyQuery {
	query := (&APIKeyGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupLinks = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withGroupLinks; query != nil {
		if err := _q.loadGroupLinks(ctx, query, nodes,
			func(n *APIKey) { n.Edges.GroupLinks = []*APIKeyGroup{} },
			func(n *APIKey, e *APIKeyGroup) { n.Edges.GroupLinks = append(n.Edges.GroupLinks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadGroupLinks(ctx context.Context, query *APIKeyGroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyGroup)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.APIKeyGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.GroupLinksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
