...
// HasOrderedAPIKeys applies the HasEdge predicate on the "ordered_api_keys" edge.
func HasOrderedAPIKeys() predicate.Group {
	return predicate.Group(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasOrderedAPIKeysWith applies the HasEdge predicate on the "ordered_api_keys" edge with a given conditions (other predicates).
func HasOrderedAPIKeysWith(preds ...predicate.APIKey) predicate.Group {
	return predicate.Group(func(s *sql.Selector) {
		step := newOrderedAPIKeysStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
