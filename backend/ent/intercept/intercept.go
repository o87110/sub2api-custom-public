...
// The UserSubscriptionCycleFunc type is an adapter to allow the use of ordinary function as a Querier.
type UserSubscriptionCycleFunc func(context.Context, *ent.UserSubscriptionCycleQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f UserSubscriptionCycleFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.UserSubscriptionCycleQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.UserSubscriptionCycleQuery", q)
}

// The TraverseUserSubscriptionCycle type is an adapter to allow the use of ordinary function as Traverser.
type TraverseUserSubscriptionCycle func(context.Context, *ent.UserSubscriptionCycleQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseUserSubscriptionCycle) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseUserSubscriptionCycle) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UserSubscriptionCycleQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.UserSubscriptionCycleQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.UserSubscriptionCycleQuery:
		return &query[*ent.UserSubscriptionCycleQuery, predicate.UserSubscriptionCycle, usersubscriptioncycle.OrderOption]{typ: ent.TypeUserSubscriptionCycle, tq: q}, nil
	default:
