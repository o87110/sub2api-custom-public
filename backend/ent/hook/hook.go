...
// The UserSubscriptionCycleFunc type is an adapter to allow the use of ordinary
// function as UserSubscriptionCycle mutator.
type UserSubscriptionCycleFunc func(context.Context, *ent.UserSubscriptionCycleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserSubscriptionCycleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserSubscriptionCycleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserSubscriptionCycleMutation", m)
}
