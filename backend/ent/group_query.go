...
type GroupQuery struct {
	...
	withOrderedAPIKeys    *APIKeyQuery
...
// QueryOrderedAPIKeys chains the current query on the "ordered_api_keys" edge.
func (_q *GroupQuery) QueryOrderedAPIKeys() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, selector),
			sqlgraph.To(apikey.Table, apikey.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, group.OrderedAPIKeysTable, group.OrderedAPIKeysPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withOrderedAPIKeys:    _q.withOrderedAPIKeys.Clone(),
...
// WithOrderedAPIKeys tells the query-builder to eager-load the nodes that are connected to
// the "ordered_api_keys" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithOrderedAPIKeys(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOrderedAPIKeys = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withOrderedAPIKeys; query != nil {
		if err := _q.loadOrderedAPIKeys(ctx, query, nodes,
			func(n *Group) { n.Edges.OrderedAPIKeys = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.OrderedAPIKeys = append(n.Edges.OrderedAPIKeys, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadOrderedAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.OrderedAPIKeysTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(group.OrderedAPIKeysPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.OrderedAPIKeysPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.OrderedAPIKeysPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "ordered_api_keys" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
