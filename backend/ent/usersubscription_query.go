...
type UserSubscriptionQuery struct {
	...
	withCycles         *UserSubscriptionCycleQuery
...
func (_q *UserSubscriptionQuery) QueryCycles() *UserSubscriptionCycleQuery {
	query := (&UserSubscriptionCycleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserSubscriptionQuery) WithCycles(opts ...func(*UserSubscriptionCycleQuery)) *UserSubscriptionQuery {
	query := (&UserSubscriptionCycleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withCycles; query != nil {
		if err := _q.loadCycles(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.Cycles = []*UserSubscriptionCycle{} },
			func(n *UserSubscription, e *UserSubscriptionCycle) { n.Edges.Cycles = append(n.Edges.Cycles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserSubscriptionQuery) loadCycles(ctx context.Context, query *UserSubscriptionCycleQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *UserSubscriptionCycle)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usersubscriptioncycle.FieldSubscriptionID)
	}
	query.Where(predicate.UserSubscriptionCycle(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.CyclesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
