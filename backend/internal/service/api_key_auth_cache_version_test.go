...
func TestAPIKeyService_RejectsV19AuthSnapshotWithoutModelBlocklist(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-model-blocklist", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v19 auth snapshot to be rejected after blocked_models was added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
