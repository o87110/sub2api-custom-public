...
type AdminSubscriptionPlanResult struct {
	...
	AllowExistingUserRenewal bool      `json:"allow_existing_user_renewal"`
...
func adminSubscriptionPlansForResponse(plans []*dbent.SubscriptionPlan, groupInfo map[int64]service.PlanGroupInfo) []AdminSubscriptionPlanResult {
	...
	for _, p := range plans {
		...
		result = append(result, AdminSubscriptionPlanResult{
			...
			ForSale:                  p.ForSale,
			AllowBulkQuotaReset:      p.AllowBulkQuotaReset,
			RemainingQuantity:        p.RemainingQuantity,
			InventoryAutoDelisted:    p.InventoryAutoDelisted,
			SoldOutAction:            p.SoldOutAction,
			AllowExistingUserRenewal: p.AllowExistingUserRenewal,
			RenewalGraceDays:         p.RenewalGraceDays,
